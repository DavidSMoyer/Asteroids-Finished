...
public class Asteroid extends SmoothMover
{
    ...
    private void setSize(int size)
    {
        stability = size;
        this.size = size;
        getImage().scale(this.size,this.size);
    }
    ...
    /**
     * Reduces the asteroid's health by a certain amount.
     * 
     * @param damage The amount to damage the asteroid.
     * @return Returns nothing.
     */
    public void hit(int damage)
    {
        ...
        if (stability <= 0)
        {
            ...
            breakUp(Greenfoot.getRandomNumber(5)+1);
        }
    }
    ...
    /**
     * Breaks the asteroid into a set amount of smaller asteroids.
     * 
     * @param numBreakUp The amount of smaller astroids to break the asteroid into.
     * @return Returns nothing.
     */
    private void breakUp(int numBreakUp)
    {
        ...
        {
            rotate = getVelocity().getDirection() + Greenfoot.getRandomNumber(45);
            length = getVelocity().getLength();
            int i = 0;
            while(i < numBreakUp)
            {
                speed = new Vector(rotate+Greenfoot.getRandomNumber(201)-100,length*1.2);
                debris = new Asteroid(size/numBreakUp,speed);
                ((Space)getWorld()).addObject(debris,getX(),getY());
                debris.move();
                i++;
            }
            ((Space)getWorld()).removeObject(this);
